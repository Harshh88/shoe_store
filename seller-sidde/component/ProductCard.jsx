...
import { ShoppingBag, Eye, AlertTriangle, Pencil, Trash2, Currency } from 'lucide-react';

export default function ProductCard({ product }) {
  const { name,  price, stock,  url } = product;
  // console.log(product)
    ...
    <div className="bg-[#141414] border border-[#222222] rounded-3xl p-5 relative overflow-hidden group hover:border-zinc-800 transition-all duration-300">
      ...
      <div className="relative w-full aspect-square bg-[#090909] rounded-2xl mb-4 overflow-hidden flex items-center justify-center border border-[#1b1b1b]">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 z-20">
          <button className="p-2.5 bg-[#141414]/90 hover:bg-zinc-800 text-zinc-300 hover:text-white rounded-full transition-all border border-[#2c2c2c] backdrop-blur-sm">
            <Pencil className="w-3.5 h-3.5" />
          </button>
          <button className="p-2.5 bg-[#141414]/90 hover:bg-red-950 text-zinc-400 hover:text-red-400 rounded-full transition-all border border-[#2c2c2c] backdrop-blur-sm">
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="text-xl font-mono font-black text-[#D9FA53]">{price.toLocaleString('en-IN',{
          style: 'currency',
          currency: 'INR'
        })}</div>
      </div>
