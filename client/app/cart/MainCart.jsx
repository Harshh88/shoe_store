...
const CheckoutComponent = ({ totalOfProduct, deleteItem, onCheckout }) => {
  ...
  // Price ko safely handle karne ke liye fallback calculations array method chain rule sync kiya hai
              ...
              <div key={item.cart_item_id} className="bg-[#0f0f0f] group p-5 rounded-2xl flex gap-6 border border-zinc-900 hover:bg-black cursor-pointer">
                ...
                <div className="flex-grow flex flex-col justify-between">
                  ...
                  <div className="flex justify-between items-center mt-4">
                    ...
                    <p className="text-lg font-black italic">₹{Number(item.product_price || 0).toLocaleString("en-IN")}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="bg-[#0f0f0f] p-8 rounded-[2.5rem] border border-zinc-900 sticky top-10">
            ...
            <div className="space-y-4 text-xs font-bold tracking-widest text-zinc-500 uppercase">
              ...
              <div className="flex justify-between border-t border-zinc-900 pt-4 mt-4 text-xl italic font-black text-white">
                ...
                <span className="text-3xl tracking-tighter">₹{Number(totalOfProduct.totalprice || subtotal).toLocaleString("en-IN")}</span>
              </div>
            </div>

            {/* Added onCheckout redirect control click handler */}
            <button 
              onClick={onCheckout}
              className="cursor-pointer w-full bg-[#F7FFB0]/80 text-black font-black py-5 rounded-2xl mt-10 uppercase tracking-widest text-xs hover:brightness-110 transition-all active:scale-95 text-center"
            >
              Checkout
            </button>
